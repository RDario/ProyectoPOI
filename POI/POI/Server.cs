...
namespace POI
{
    public class Server
    {
        ...
        public static String ADDRESS = "10.1.112.68";
        public List<Grupo> mGrupos { get; set; }
                                    ...
                                    while (mIsThreadRunning)
                                    {
                                        ...
                                        if (command.Contains("<SendMessage>"))
                                        {
                                            ...
                                            foreach (Client c in mClients)
                                            {
                                                ...
                                                {
                                                    String cmd = "<IncomingMessage><size>" + '0' + "<size><name>"
                                        ...
                                        if (command.Contains("<SendGroupMessage>"))
                                        {
                                            String[] mainSplit = command.Split(new String[] { "<SendMessage>" }, StringSplitOptions.RemoveEmptyEntries);
                                            string[] contentSplit = mainSplit[0].Split(new String[] { "<size>", "<group>", "<message>" }, StringSplitOptions.RemoveEmptyEntries);
                                            int receiveSize = Int32.Parse(contentSplit[0]);
                                            String toGroupName = contentSplit[1];
                                            String message = contentSplit[2];
                                            String message1 = client.mName + ": " + message;
                                            StreamWriter escrito;
                                            if (File.Exists("Conversaciones/" + toGroupName + ".txt")) {
                                                escrito = File.AppendText("Conversaciones/" + toGroupName + ".txt");
                                            } else {
                                                escrito = File.AppendText("Conversaciones/" + toGroupName + ".txt");
                                            }
                                            String contenido = "\r\n" + message1;
                                        ...
                                        else if (command.Contains("<Chatgrupal>"))
                                        {
